...
module.exports = (bot) => {
    ...
    bot.command('id', isAdmin, async (ctx) => {
        try {
            ...
            if (!order) {
                return ctx.reply(`⚠️ Bestellung "${args}" nicht gefunden.`);
            }
            ...
            if (order.shipping_link) {
                text += `\n📦 Versandadresse: [Privnote öffnen](${order.shipping_link})`;
            }
            if (order.payment_link) {
                text += `\n🔗 TX-Link: [Privnote öffnen](${order.payment_link})`;
            }

            // Admin-Notizen anzeigen
            if (order.admin_notes && order.admin_notes.length > 0) {
                text += `\n\n📝 *Notizen:*`;
                order.admin_notes.forEach((note, i) => {
                    const noteDate = new Date(note.date).toLocaleString('de-DE', { timeZone: 'Europe/Berlin' });
                    text += `\n${i + 1}. _${note.author}_ (${noteDate}): ${note.text}`;
                });
            }
            ...
            const keyboard = {
                inline_keyboard: [
                    ...
                    [
                        { text: '⚙️ In Bearbeitung', callback_data: `order_status_${order.order_id}_in_bearbeitung` },
                        { text: '📦 Versendet', callback_data: `order_status_${order.order_id}_versand` }
                    ],
                    [
                        { text: '✅ Abgeschlossen', callback_data: `order_status_${order.order_id}_abgeschlossen` },
                        { text: '❌ Abgebrochen', callback_data: `order_status_${order.order_id}_abgebrochen` }
                    ],
                    [{ text: '📝 Notiz hinzufügen', callback_data: `order_note_${order.order_id}` }],
                    [{ text: '🗑 Bestellung löschen', callback_data: `order_delete_${order.order_id}` }]
                ]
            };
    ...
    bot.command('orders', isAdmin, async (ctx) => {
        try {
            ...
            orders.forEach((order, i) => {
                ...
                text += `${i + 1}. \`${order.order_id}\` | ${username} | ${formatters.formatPrice(order.total_amount)} | ${texts.getStatusLabel(order.status)} | ${date}\n`;
            });
    ...
    bot.command('ban', isAdmin, async (ctx) => {
        try {
            ...
            // Schutzprüfungen
            if (targetId === userId) {
                return ctx.reply(texts.getBanSelfError());
            }
            if (targetId === Number(config.MASTER_ADMIN_ID)) {
                return ctx.reply(texts.getBanMasterError());
            }

            // Prüfen ob User bereits gebannt
            const alreadyBanned = await userRepo.isUserBanned(targetId);
            if (alreadyBanned) {
                return ctx.reply(texts.getBanAlreadyBanned());
            }

            // User bannen
            await userRepo.banUser(targetId);

            // Pending Ban erstellen (48h Wartezeit)
            ...
            // Chatverlauf löschen (soweit möglich via Telegram API)
            try {
                await bot.telegram.sendMessage(targetId, texts.getBannedMessage()).catch(() => {});
            } catch (e) {
                // Ignorieren falls Chat nicht erreichbar
            }
