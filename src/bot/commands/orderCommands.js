...
module.exports = (bot) => {
    ...
    bot.command('deleteid', isAdmin, async (ctx) => {
        try {
            ...
            ctx.reply(`🗑 Bestellung \`${args}\` gelöscht.`, { parse_mode: 'Markdown' });
        } catch (error) {
    ...
    bot.command('orders', isAdmin, async (ctx) => {
        try {
            ...
            await ctx.reply(text, {
                parse_mode: 'Markdown',
                reply_markup: { inline_keyboard: [[{ text: '🗑 ALLE löschen', callback_data: 'orders_delete_all_confirm' }]] }
            });
        } catch (error) {
            console.error('Orders Error:', error.message);
            ctx.reply('❌ Fehler beim Laden der Bestellungen.');
        }
    });
